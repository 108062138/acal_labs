...
class CpxCal extends Module{
    ...
    val sFIRST_IDLE:: sACCEPT :: sPOS :: sSEE_LP :: sNEG :: sFIRST_DEMO :: sSECOND_IDLE :: sREMOVE_PARA :: sFIND_SMALLER_OPERATOR :: sLOOKING_FOR_LP :: sDUMP :: sSECOND_DEMO :: sTHIRD_IDLE :: sCOMPUTE::sFINISH ::Nil = Enum(15)
    ...
    val removed_neg_tokens_sp = RegInit(0.U(log2Ceil(56 + 1).W))
    val next_removed_neg_tokens_sp = Wire(UInt(log2Ceil(56 + 1).W))

    val removed_para_tokens_stack = Mem(56, UInt(36.W))
    val removed_para_tokens_sp = RegInit(0.U(log2Ceil(56 + 1).W))
    val next_removed_para_tokens_sp = Wire(UInt(log2Ceil(56 + 1).W))

    val operator_tokens_stack = Mem(56, UInt(36.W))
    val operator_tokens_sp = RegInit(0.U(log2Ceil(56 + 1).W))
    val next_operator_tokens_sp = Wire(UInt(log2Ceil(56 + 1).W))

    val my_stack = Mem(56, UInt(64.W))
    val my_tokens_sp = RegInit(0.U(log2Ceil(56 + 1).W))
    val next_my_tokens_sp = Wire(UInt(log2Ceil(56 + 1).W))

    val travel_removed_neg_sp = RegInit(0.U(log2Ceil(56 + 1).W))
    val next_travel_removed_neg_sp = Wire(UInt(log2Ceil(56 + 1).W))

    val enum_sp = RegInit(0.U(log2Ceil(56 + 1).W))
    val next_enum_sp = Wire(UInt(log2Ceil(56 + 1).W))
    ...
    my_tokens_sp := next_my_tokens_sp
    enum_sp := next_enum_sp
    ...
    next_my_tokens_sp := my_tokens_sp
    next_enum_sp := enum_sp
    ...
    // handle my_stack/enum_sp/my_tokens_sp
    val a = Wire(SInt(64.W))
    val b = Wire(SInt(64.W))
    val c = Wire(SInt(128.W))
    ...
    switch(state){
        is(sTHIRD_IDLE){
            next_my_tokens_sp := 0.U
            next_enum_sp := 0.U
        }
        is(sCOMPUTE){
            next_enum_sp := enum_sp + 1.U
            when(removed_para_tokens_stack(enum_sp)(35,32) === 0.U){
                next_my_tokens_sp := my_tokens_sp + 1.U
                my_stack(my_tokens_sp) := removed_para_tokens_stack(enum_sp)(31, 0).asSInt.asTypeOf(SInt(64.W)).asUInt
            }.otherwise{
                // pop two items out
                next_my_tokens_sp := my_tokens_sp - 1.U

                b := my_stack(my_tokens_sp - 1.U).asSInt
                a := my_stack(my_tokens_sp - 2.U).asSInt

                when(removed_para_tokens_stack(enum_sp)(35,32) === sADD){
                    c := a + b
                }.elsewhen(removed_para_tokens_stack(enum_sp)(35,32) === sMINUS){
                    c := a - b
                }.elsewhen(removed_para_tokens_stack(enum_sp)(35,32) === sMUL){
                    c := a * b
                }

                printf(p"a: $a, b: $b, c: $c\n")

                my_stack(my_tokens_sp - 2.U) := c(63, 0).asUInt
            }
        }
    }
    ...
    switch(state){
        ...
        is(sCOMPUTE){
            when(enum_sp < removed_para_tokens_sp - 1.U){
                next_state := sCOMPUTE
            }.otherwise{
                next_state := sFINISH
            }
